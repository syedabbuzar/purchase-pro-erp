...
export const Route = createFileRoute("/_authenticated/invoices")({
  component: Invoices,
  head: () => ({ meta: [{ title: "Invoices — STAR ENTERPRISES" }] }),
});
              ...
              {filtered.map((i) => {
                ...
                return (
                  <tr key={i.id} className="border-t hover:bg-muted/30">
                    <td className="p-2"><Link to="/invoice-preview/$id" params={{ id: String(i.id) }} className="text-primary font-medium hover:underline">{i.number}</Link></td>
                    ...
                    <td className="text-right pr-2"><Link to="/invoice-preview/$id" params={{ id: String(i.id) }} className="text-primary hover:underline">Open</Link></td>
                  </tr>
                );
              })}

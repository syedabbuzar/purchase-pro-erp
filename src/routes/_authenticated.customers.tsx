...
export const Route = createFileRoute("/_authenticated/customers")({
  component: Customers,
  head: () => ({ meta: [{ title: "Customers — STAR ENTERPRISES" }] }),
});
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id} className="border-t hover:bg-muted/30">
                  <td className="p-2"><Link to="/customers/$id" params={{ id: String(c.id) }} className="font-medium text-primary hover:underline">{c.name}</Link></td>
                  <td>{c.shopName || "—"}</td>
                  <td>{c.mobile}</td>
                  <td>{c.gstin || "—"}</td>
                  <td>{c.city || "—"}</td>

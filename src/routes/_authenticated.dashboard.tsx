...
export const Route = createFileRoute("/_authenticated/dashboard")({
  component: Dashboard,
  head: () => ({ meta: [{ title: "Dashboard — STAR ENTERPRISES" }] }),
});
                ...
                {data.recent.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b last:border-0 hover:bg-muted/50">
                      <td className="py-2"><Link to="/invoice-preview/$id" params={{ id: String(r.id) }} className="text-primary hover:underline">{r.number}</Link></td>
